...
class SmallNumber {
 public:
  ...
  SmallNumber(float val) : val_(std::min(std::max(val * 16.0f, 0.0f), 65535.0f)) {}
  SmallNumber(double val) : val_(std::min(std::max(val * 16.0, 0.0), 65535.0)) {}
  ...
  uint16_t ceil() const { return ((uint32_t)val_ + 15) >> 4;}// return 4096 - ((65536 - val_) >> 4); }
