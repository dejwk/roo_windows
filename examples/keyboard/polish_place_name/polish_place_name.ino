// Enter a Polish place name: hold a, c, e, l, n, o, s, or z for accented
// letters.
...
material3::TextField place(app.context(), "Nazwa miejsca");

void setup() {
  InitDisplay();
  app.keyboard().setLayout(kbPolPLLayout());
  place.setSupportingText("Przytrzymaj litere, aby wybrac polski znak");
  form.setPadding(Padding(Scaled(8)));
  form.add(place);
  app.addTaskFullScreen(form);
  app.start();
  scheduler.run();
}
