...
def annotated_rows(source, data):
    """Render each fixed record on one line, annotating its meaning and offset."""
    lines = []
    def read(offset, size):
        return int.from_bytes(data[offset:offset + size], 'big')
    def section(offset, description):
        lines.append(f'    // 0x{offset:04X}: {description}')
    def record(offset, size, comment):
        values = ', '.join(f'0x{b:02X}' for b in data[offset:offset + size])
        lines.append(f'    {values},  // {comment}')
    def character(value):
        # repr escapes control characters, quotes and backslashes in comments.
    ...
    for p, page in enumerate(source['pages']):
        offset = read(10 + p * 4, 2)
        section(offset, f"Rows for {page['id']!r}: key count, reserved, keys offset:u16")
        for row in range(len(page['rows'])):
            record(offset + row * 4, 4, f'row {row}')
    menus, labels = set(), set()
    actions = {value: name for name, value in ACTIONS.items()}
    for p, page in enumerate(source['pages']):
        rows = read(10 + p * 4, 2)
        for row in range(len(page['rows'])):
            offset = read(rows + row * 4 + 2, 2)
            section(offset, f"Keys for {page['id']!r}, row {row}")
            for k in range(data[rows + row * 4]):
                pos = offset + k * 11
                function = data[pos + 2] & 7
                if function == 0:
                    comment = pair(pos + 3)
                else:
                    comment = actions[function]
                    if function == ACTIONS['switch_page']:
                        target = read(pos + 3, 3)
                        label = read(pos + 6, 3)
                        labels.add(label)
                        text = data[label + 1:label + 1 + data[label]].decode('utf-8')
                        comment += f" {text!r} -> {source['pages'][target]['id']!r}"
                    if data[pos + 2] & 8:
                        comment += ', circle'
                menu = read(pos + 9, 2)
                if menu:
    ...
    for offset in sorted(labels):
        ...
        for i in range(0, data[offset], 12):
            size = min(12, data[offset] - i)
            record(offset + 1 + i, size, repr(data[offset + 1 + i:offset + 1 + i + size]))
    return '\n'.join(lines)
...
'''
    rows = annotated_rows(source, data)
