...
class SingleLineText : public roo_display::Drawable {
 public:
  SingleLineText(const roo_display::Font& font, roo_display::Font::Options options,
                 roo_display::Box bounds, roo::string_view text, bool masked,
                 bool show_last, int16_t offset, int16_t highlight_begin,
                 int16_t highlight_end, roo_display::Color color,
                 roo_display::Color highlight)
      : font_(font), options_(options), bounds_(bounds), text_(text),
        masked_(masked), show_last_(show_last), offset_(offset),
        begin_(highlight_begin + offset), end_(highlight_end + offset),
        color_(color), highlight_(highlight) {}
  roo_display::Box extents() const override { return bounds_; }
 private:
  void drawText(const roo_display::Surface& surface) const {
    using namespace roo_display;
    if (!masked_) {
      surface.drawObject(MakeTileOf(StringViewLabel(text_, font_, color_, options_),
          bounds_, kOrigin.shiftBy(offset_) | kBaseline));
      return;
    }
    ...
    auto blank = [&](int16_t left, int16_t right) {
      ...
      surface.drawObject(MakeTileOf(StringViewLabel("", font_, color_),
    ...
    while (decoder.next(rune)) {
      ...
      char encoded[4];
      int count = roo_io::WriteUtf8Char(encoded, last && show_last_ ? rune : U'*');
      ...
      clipped.drawObject(MakeTileOf(
          StringViewLabel(roo::string_view(encoded, count), font_, color_, options_),
  ...
  void drawTo(const roo_display::Surface& surface) const override {
    ...
    auto band = [&](int16_t left, int16_t right, bool highlighted) {
      Box clip = Box::Intersect(bounds_, Box(left, bounds_.yMin(), right, bounds_.yMax()));
      if (clip.empty()) return;
      Surface part = surface;
      part.clipToExtents(clip);
      if (highlighted) part.set_bgcolor(AlphaBlend(surface.bgcolor(), highlight_));
      drawText(part);
    };
    if (end_ < begin_) { band(bounds_.xMin(), bounds_.xMax(), false); return; }
