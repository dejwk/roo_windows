...
class Material3CheckboxGoldenTest : public testing::Test {
 protected:
  static constexpr int16_t kWidth = 108;
  static constexpr int16_t kHeight = 40;
  static constexpr int16_t kBoxSize = Scaled(18);
  static constexpr int16_t kRowY = 11;
  static constexpr int16_t kX0 = 12;
  static constexpr int16_t kGap = 18;
  static constexpr int16_t kStride = kBoxSize + kGap;

  Material3CheckboxGoldenTest()
  ...
  void AddCheckbox(Application& app, int16_t x, OnOffState state,
                   bool enabled) {
    auto checkbox = std::make_unique<Checkbox>(env_, state);
    checkbox->setEnabled(enabled);
    app.add(std::move(checkbox),
            roo_display::Box(x, kRowY, x + kBoxSize - 1, kRowY + kBoxSize - 1));
  }
...
TEST_F(Material3CheckboxGoldenTest, EnabledStatesRow) {
  ...
  EXPECT_TRUE(test::CompareOrUpdateGolden(
      image, "test/goldens/material3_checkbox/enabled_states_row.ppm",
...
TEST_F(Material3CheckboxGoldenTest, DisabledStatesRow) {
  ...
  EXPECT_TRUE(test::CompareOrUpdateGolden(
      image, "test/goldens/material3_checkbox/disabled_states_row.ppm",
      "material3_checkbox_disabled_states_row"));
}
