...
| [HorizontalPageHost](../src/roo_windows/containers/horizontal_page_host.cpp) | Context scheduler, 10 ms callbacks, 180 ms quadratic ease-out settling; gesture handoff, page position and slot/cache updates | Needs interruption, retargeting, coupled geometry updates, and a frame callback that precedes layout/paint |
...
| [ExpandablePanel](../src/roo_windows/material3/list/list.cpp) | Measurement advances progress by a fixed 20 ms step; painting requests the next layout/invalidation | Duration currently depends on layout advancement. Strong candidate for elapsed-time animation and batched relayout |
