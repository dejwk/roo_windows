...
| [Material switch](../src/roo_windows/material3/switch/switch.cpp), [legacy switch](../src/roo_windows/widgets/switch.cpp) | Registry value tracks drive 100 ms Material and 120 ms legacy thumb motion; each widget retains one packed logical-state/applied-fraction word | Programmatic setters snap, rapid clicks retarget from the applied fraction, and ClickAnimation remains an independent overlay channel; active registry cost is explicit rather than an assumed RAM saving |
| [Toggle icon button](../src/roo_windows/material3/button/toggle_icon_button.cpp) | Registry value track drives the independent 100 ms selection morph; the shared click controller still owns press/click appearance | Selection timing continues while click geometry has precedence; rapid programmatic changes retarget continuously, while hidden/detached buttons cancel and snap to selected rest geometry |
...
| [Tabs and ScrollableTabs](../src/roo_windows/material3/tabs/tabs.cpp) | Registry tag 0 drives the 200 ms indicator value track; derived tag 1 drives selected-item reveal, fling, and spring-back from 10 ms custom-time samples | Indicator and strip motion share a logical frame but remain independently replaceable/cancelable; neither tabs class stores a scheduler ticket or timestamp |
| [SimpleScrollablePanel](../src/roo_windows/containers/scrollable_panel.cpp) and [scroll motion](../src/roo_windows/containers/scroll_motion_controller.cpp) | Registry custom-time samples drive fling and spring-back at the existing 10 ms minimum interval. A separate one-shot scheduler deadline hides transient scroll chrome | Each motion uses track-relative elapsed time from a zero epoch; settling removes its channel, while hidden/detached panels cancel and clamp instead of retaining momentum |
...
| [SnackbarHost/Presenter](../src/roo_windows/material3/snackbar/snackbar.cpp) | Shared value track for 150/100 ms entry/exit; one semantic readable-time deadline. Hidden/empty/modal state is event-driven; control focus pauses only readable time | Migrated. Queue ownership, remaining-time timeout policy, focus/modal reconciliation, and completion reasons remain presenter responsibilities; no private frame driver remains |
| [Legacy ProgressBar](../src/roo_windows/widgets/progress_bar.cpp) | Registry custom-time track supplies a stored phase for the 1424 ms marquee waveform | Indeterminate motion begins at phase zero per presentation, cancels for hidden/detached/empty bounds, and no longer depends on paint frequency or global uptime |
| [TextFieldEditor and TextField](../src/roo_windows/widgets/text_field.cpp) | The task-local editor stores caret visibility while a 500 ms custom-time channel is registered on its active field; password reveal remains a one-shot scheduler deadline | Binding, focus transfer, editing, and presentation lifecycle restart or cancel one sparse caret channel without adding state to every field or polling at display cadence |
| [Material progress indicators](design/implemented/material3_progress_indicators_design.md) | Implemented custom-time registry client with a stable published phase and explicit reduced motion | Standard Material waveforms, bounded foreground invalidation, no private timer |
