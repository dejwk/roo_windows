...
| Display runtime Phase 4 optional navigation | Phases 4a through 4c are implemented: direct tasks borrow one fixed content widget, and optional `NavigationHost` instances borrow destinations with growable history, Back routing, and Activity-compatible destination lifecycle hooks. Legacy-host isolation remains proposed. |
