...
| Dialogs | Legacy dialogs, icon buttons, and shared Back behavior exist. P1.6b migrates legacy dialogs to preconfigured measurable content, explicit task ownership, and the one shared structural host, then removes their dialog-specific `MainWindow` path. Basic Material 3 dialogs remain P1.8 behind P1.6b, and full-screen dialogs remain a later phase. |
