...
| Transient surface hosting | Phases 1–4 implement presenter-owned focus scopes, the owner-bound composite host, source capture, policy preflight, hosted replacement, paint composition, shutdown-safe cleanup, display-wide input isolation, and owner-scoped copied-geometry pins. Prepared admission and legacy-dialog migration remain. |
...
| Dialogs | Legacy dialogs, icon buttons, and shared Back behavior exist. P1.6b gives legacy dialogs guarded preparation with persistent or presentation-scoped content, explicit task ownership, and the one shared structural host, then removes their dialog-specific `MainWindow` path. Basic Material 3 dialogs remain P1.8 behind P1.6b, and full-screen dialogs remain a later phase. |
