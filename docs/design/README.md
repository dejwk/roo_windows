...
| Display runtime Phase 4 optional navigation | Phases 4a and 4b are implemented: direct tasks borrow one fixed content widget, and optional `NavigationHost` instances borrow destinations with growable history and generation-checked Back routing. Activity-compatible destination lifecycle and legacy-host isolation remain proposed. |
