...
Implementation adjustment: AOSP Polish has nine alternatives for `a`; version 2
accepts nine alternatives, without inserting the base character. Static C++ entry
...
The [keyboard renderer](../../../src/roo_windows/keyboard/keyboard.cpp) already
...
An **alternative list** declares its row count and default index, followed by an ordered list of lowercase/uppercase scalar pairs
      ...
      "rows": [
        {
          ...
          "keys": [
            ...
            {"text": "e", "upper": "E", "width": 2,
             "alternative_rows": 1, "default_alternative": 0,
             "alternatives": [
               {"text": "é", "upper": "É"},
               {"text": "è", "upper": "È"},
               {"text": "ę", "upper": "Ę"}
...
Holding `e` presents `é è ę`; holding shifted `E` presents `É È Ę`.
The original letter is not inserted. The declared default (index 0 here) is
selected initially; holding and releasing commits `é` or `É`.
...
python3 tools/keyboard_layout/compile.py layouts/accent_demo.json \
  --output-prefix src/roo_windows/keyboard/layout/accent_demo
```
...
order. Key records are not deduplicated in version 2.

`format` must be 2. `name` is a lowercase, underscore-separated identifier. Page IDs are unique strings;
...
Version 2 accepts 1–255 pages, 1–255 rows per page, and widths of 1–255 units.
...
Each text key accepts at most nine alternatives, giving at most nine choices
without inserting the base. Alternative objects accept only `text` and `upper`; recursive
submenus are rejected. Keys with alternatives require `alternative_rows` and
`default_alternative`, a zero-based index in the bottom row. With
`columns = ceil(count / rows)`, rows must be occupied and columns at most five.
Unknown fields, duplicate JSON object members, invalid
...
| --- | --- | ---: |
| Header | magic `RWKB` (4 bytes), version `u8=2`, page count `u8`, total size `u16` | 8 |
...
| Alternatives | count `u8`, rows `u8`, default index `u8`, then count pairs `(low u24, high u24)` | 3 + 6 × count |
...
A zero offset means no alternatives. No base choice is injected into the popup.
...
empty output view. Version 2 exposes no external file-loading API or checksum.
...
declared default alternative.
The popup is a rounded grid with at most five choices per row, with
only authored alternatives, in array order left-to-right then top-to-bottom. It has
...
independently of the outer padding. Read `rows` from the menu and compute
`columns = ceil(n / rows)`. The compiler and reader require occupied rows,
at most five columns, and a default index in the bottom row.

Anchor the declared default's center above the held key. If this clips
horizontally, clamp the popup into the viewport without changing the default.
For pixel parity differences use the nearest representable center.
...
cannot fit entirely above the key, or its width exceeds the viewport,
...
moving horizontally there selects the corresponding bottom-row choice relative
to the original key and declared default, even when the visible pin shifted. There
...
class KeyboardLayout {
 public:
  ...
  struct Key {
    ...
    uint8_t alternative_count = 0;  // No implicit base character.
    uint8_t alternative_rows = 0;
    uint8_t default_alternative = 0;  // Zero-based index in the bottom row.
  };
...
struct AlternativeSelection {
  ...
  int8_t selected = -1;  // 0..8 = alternatives, -1 = none.
...
`KeyboardLayout` is the sole reader in `keyboard/layout/keyboard_layout.h/.cpp`; generated byte
...
107 bytes (8 header + 4 page + 8 rows + 66 keys + 21 alternatives). The byte
...
authored default, uppercase accents, caps lock, outside cancellation, popup fit and
...
Version 2 deliberately bounds layouts to 64 KiB minus one byte and alternatives
