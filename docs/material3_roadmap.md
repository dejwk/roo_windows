...
- [material3_layout_scaffold_design.md](design/implemented/material3_layout_scaffold_design.md)
- [display_key_event_bindings_design.md](design/implemented/display_key_event_bindings_design.md)
- [back_request_coordination_design.md](design/implemented/back_request_coordination_design.md)
- [transient_presenter_lifetime_ownership_design.md](design/in_progress/transient_presenter_lifetime_ownership_design.md)
- [transient_presentation_pins_design.md](design/in_progress/transient_presentation_pins_design.md)
- [transient_surface_hosting_design.md](design/implemented/transient_surface_hosting_design.md)
...
Designs already cover the shell and many major families, including the
implemented adaptive scaffold,
...
| <a id="p0-3"></a>[P0.3](#p0-3) | Keyboard/focus framework | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Non-touch input](design/implemented/non_touch_input_design.md) | — | Key acquisition, focused-widget lifetime, traversal, control operation, structured navigation, hardware text entry, and one-level presenter focus-scope activation are covered by implemented tests. |
| <a id="p0-4"></a>[P0.4](#p0-4) | Semantic Back/Escape routing | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Back request coordination](design/implemented/back_request_coordination_design.md) | P0.3 | UI and hardware Back/Escape share the semantic request path; current widget-first dispatch, explicit caller/key-source task routing, transient fallback, task fallback, and editor fallback are tested. Hosted root-first physical ordering remains part of P1.6b. |
| <a id="p0-5"></a>[P0.5](#p0-5) | Interactive-transient slot and dialog lifetime migration | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | Phases 1 and the dialog part of Phase 2 in [Transient presenter lifetime and ownership](design/in_progress/transient_presenter_lifetime_ownership_design.md) | P0.4 | Slot occupancy, replacement reentrancy, Back policy, host/presenter destruction, detach-before-completion, and legacy-dialog adoption tests pass. Menu, sheet, and snackbar adoption are explicitly later rows. |
...
| <a id="p1-5"></a>[P1.5](#p1-5) | Implement the shared presentation-pin host | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | Phase 1 of [Transient presentation pins](design/in_progress/transient_presentation_pins_design.md) | — | Root-stage registration, ordering, old/new-bounds invalidation, hide/unregister, anchor teardown, and window teardown tests pass. Slider/range-slider indicators and keyboard press highlights are active-only pins; the keyboard case also verifies popup ownership, focus preservation, and escaped rendering. |
| <a id="p1-6"></a>[P1.6](#p1-6) | Reconcile menu ownership and input design | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Menus](design/implemented/material3_menus_design.md), reconciled against Phase 5 of [Transient presenter lifetime and ownership](design/in_progress/transient_presenter_lifetime_ownership_design.md) and Phase 12 of [Lists](design/implemented/material3_lists_design.md) | P0.3–P0.5 | The design synchronously copies placement geometry from a live widget source, uses presenter-owned child levels, one root transient registration, deepest-first Back/Escape, `ListItem`/`ListEntry` reuse, and closed keyboard semantics. Framework hosting prerequisites are separated into P1.6a–P1.6b. |
| <a id="p1-6a"></a>[P1.6a](#p1-6a) | Review shared transient hosting | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Transient surface hosting](design/implemented/transient_surface_hosting_design.md) | P1.6 | The review closes one combined host layer, explicit interaction ownership, independent barrier-paint, outside-interaction, and admission policies, display-wide pointer/key barriers, mandatory presenter-owned focus integration, synchronous source validation, deterministic teardown, RAM budgets, and tests without creating a `Task`. Phase 7 is reconciled as task-bounded coverage of the same host. |
...
| <a id="p1-11"></a>[P1.11](#p1-11) | Integrate the compact settings shell | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | No new design; integration of P1.4, P1.7, P1.8, and P1.10 | P1.4, P1.7, P1.8, P1.10 | The shared [settings shell](../examples/material3/settings_shell/settings_shell.h) covers General/Network screens, a mode menu, reset confirmation/cancellation, focus restoration, recognized taps, physical Tab/Enter/Escape, full-screen detail navigation, and snackbar results. [Compact-target acceptance](material3_phase1_acceptance.md) records passing Arduino emulation tests and the ESP32-C3 cross-build, RAM/code/stack and repaint evidence. |

Phase 1 is complete; see the [acceptance report](material3_phase1_acceptance.md).
...
| ID | Deliverable | State | Design | Depends on | Completion check |
| --- | --- | --- | --- | --- | --- |
| <a id="p2-1"></a>[P2.1](#p2-1) | Implement single-line Material 3 text fields | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Text fields](design/implemented/material3_text_fields_design.md) | P0.3 | Phases 1–5 implemented: shared task editor, filled/outlined and secure fields, explicit activation, UTF-8 selection/scrolling, affordances, tests, goldens and example. [Acceptance](material3_text_fields_acceptance.md) records target cost and the explicitly deferred roo_display paint allocations. Multiline and IME composition remain out of scope. |
| <a id="p2-2"></a>[P2.2](#p2-2) | Design progress indicators | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Progress indicators](design/implemented/material3_progress_indicators_design.md) | P0.1 | Reviewed design defines determinate/indeterminate linear and circular variants, animation scheduling and cancellation, visibility/teardown behavior, reduced-motion policy, tokens, RAM/flash budgets, tests, and implementation phases. |
| <a id="p2-3"></a>[P2.3](#p2-3) | Implement progress indicators | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Progress indicators](design/implemented/material3_progress_indicators_design.md), with [Presentation registry](design/implemented/presentation_registry_design.md) and [Widget animation registry](design/implemented/widget_animation_registry_design.md) prerequisites | P2.2 | Linear/circular determinate and indeterminate components, reduced motion, catalog, lifecycle/composition tests, 75%/100% goldens, bounded output, zero warmed allocations, and target-cost gates pass; see [acceptance](material3_progress_acceptance.md). |
...
| <a id="p3-3"></a>[P3.3](#p3-3) | Design adaptive navigation orchestration | <img src="material3_roadmap_status_open.svg" width="20" height="20" alt="Open" title="Open"> | **Missing design:** define route ownership and breakpoint-driven switching across bar, rail, and drawer | P1.4 | Reviewed design names the route-state owner, maps compact/medium/expanded presentations, defines focus transfer and Back behavior, and forbids navigation widgets from owning route history. |
| <a id="p3-4"></a>[P3.4](#p3-4) | Implement adaptive navigation | <img src="material3_roadmap_status_pending.svg" width="20" height="20" alt="Pending" title="Pending"> | P3.3 plus the implemented [Navigation rail](design/implemented/material3_navigation_rail_design.md) and proposed [Navigation drawer](design/proposed/material3_navigation_drawer_design.md) designs | P3.3 | Bar, rail, and drawer present the same route state across size changes; focus and selection survive transitions; compact-to-expanded integration and target tests pass. |
...
| <a id="p3-7"></a>[P3.7](#p3-7) | Reconcile and implement modal sheets on the shared host | <img src="material3_roadmap_status_open.svg" width="20" height="20" alt="Open" title="Open"> | [Sheets](design/proposed/material3_sheets_design.md) and Phases 3–4 of [Transient presenter lifetime and ownership](design/in_progress/transient_presenter_lifetime_ownership_design.md) | P0.5, P1.4, P1.6b | Reconcile the sheet design before implementation. Modal sheet presenters use the shared host with an explicit interaction owner, host-owned scrim barrier, presenter-owned focus scope, and presenter-handled outside interaction so close animation finishes before teardown; content detaches before completion, and gesture, Back, lifetime, golden, and target tests pass. |
...
| Icon buttons | [Implemented design](design/implemented/material3_icon_buttons_design.md) | The non-toggle family has unit, golden, size-budget, and compact-controls toolbar coverage. Future work can adopt it in an app bar, menu anchor, or badge-aware host. |
| Toggle icon buttons | [Implemented design](design/implemented/material3_toggle_icon_buttons_design.md) | The family has focused unit and rendering coverage plus compact-controls persistent-preference adoption. Independent selection semantics remain until button-group policy is scheduled. |
| Toolbars, FABs, extended FABs, split buttons, and button groups | Proposed designs exist | Icon buttons are available; select a concrete consuming flow and schedule only the component families it requires. |
