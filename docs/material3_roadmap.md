...
| ID | Deliverable | State | Design | Depends on | Completion check |
| --- | --- | --- | --- | --- | --- |
| <a id="p1-1"></a>[P1.1](#p1-1) | Finish app-bar component verification | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [App bars and search surfaces](design/in_progress/material3_app_bars_design.md) | P0.1, P0.3 | `material3_app_bar_golden_test` covers title variants, standalone search, and flat/scrolled search app bars; the unit test and example pass. Scaffold integration remains P1.4. |
| <a id="p1-2"></a>[P1.2](#p1-2) | Amend the navigation-bar design for keyboard operation | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | Revise [Navigation bar](design/proposed/material3_navigation_bar_design.md) | P0.3 | The design explicitly defines Tab entry/exit, arrow-key movement, selection versus focus, Enter/Space activation, disabled destinations, focus restoration, and tests using the implemented focus manager. |
...
Known P1.1 follow-up defect:

- Animated area ripples on rounded search surfaces can leave stale horizontal
  bands where a child paints in the parent's pill-decoration area. A future
  fix must preserve the ability for children to paint anywhere inside the pill
  and apply the parent's transient ripple while those child pixels are drawn;
  clipping children to the pill's inner rectangle is not an acceptable fix.
