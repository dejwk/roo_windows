...
- [transient_surface_hosting_design.md](design/proposed/transient_surface_hosting_design.md)
...
behavior, and admission remain independent policies. Menus provide live widget
sources at `show()` so placement and optional trigger-paint geometry are copied
synchronously and rect pins can use the existing presentation-pin host.
P1.6b also migrates legacy dialogs to preconfigured measurable roots, an
explicit interaction owner, and the same composite host, then removes the
dialog-specific `MainWindow` path. Implementation is scheduled immediately
before menus.
...
| <a id="p1-6"></a>[P1.6](#p1-6) | Reconcile menu ownership and input design | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Menus](design/proposed/material3_menus_design.md), reconciled against Phase 5 of [Transient presenter lifetime and ownership](design/in_progress/transient_presenter_lifetime_ownership_design.md), Phase 12 of [Lists](design/in_progress/material3_lists_design.md), and P1.5 | P0.3–P0.5, P1.5 | The design synchronously copies placement and trigger-paint geometry from live widget sources, uses presenter-owned child levels, one root transient registration, deepest-first Back/Escape, `ListItem`/`ListEntry` reuse, and closed keyboard semantics. Framework hosting prerequisites are separated into P1.6a–P1.6b. |
| <a id="p1-6a"></a>[P1.6a](#p1-6a) | Review shared transient hosting | <img src="material3_roadmap_status_completed.svg" width="20" height="20" alt="Completed" title="Completed"> | [Transient surface hosting](design/proposed/transient_surface_hosting_design.md) | P1.6 | The review closes one combined host layer, explicit interaction ownership, independent barrier-paint, outside-interaction, and admission policies, display-wide pointer/key barriers, mandatory presenter-owned focus integration, synchronous source validation, presenter-pin integration, deterministic teardown, RAM budgets, and tests without creating a `Task`. Phase 7 is reconciled as task-bounded coverage of the same host. |
| <a id="p1-6b"></a>[P1.6b](#p1-6b) | Implement shared transient hosting | <img src="material3_roadmap_status_pending.svg" width="20" height="20" alt="Pending" title="Pending"> | The implementation plan in [Transient surface hosting](design/proposed/transient_surface_hosting_design.md), consuming the existing [Non-touch input](design/implemented/non_touch_input_design.md) focus contract and [presentation-pin](design/in_progress/transient_presentation_pins_design.md) host | P1.6a | Active owner-scoped focus tests pass; the combined host layer enforces owner/non-owner physical-key and semantic-editor barriers, hosted root-first Back/Escape, and owner teardown; detached and foreign required placement sources fail before admission, invalid optional trigger sources omit only retained paint, and rect pins preserve widget-pin behavior. Legacy dialogs require explicit task ownership and preconfigured measurable content, use the same host, and no dialog-specific `MainWindow` attachment path remains. Host/layer/dialog size ceilings hold. |
...
| <a id="p1-8"></a>[P1.8](#p1-8) | Implement basic Material 3 dialogs | <img src="material3_roadmap_status_pending.svg" width="20" height="20" alt="Pending" title="Pending"> | Phases 1–2 of [Dialogs](design/proposed/material3_dialogs_design.md) | P0.4, P0.5, P1.6b | Basic/alert dialogs use the shared display-covered host with an explicit interaction owner, detach the complete hosted root before completion, retain configured body state for reopen, restore focus, handle Back/Escape, and pass unit and golden tests. Suitable in-repository legacy alert and radio-list call sites migrate to the Material 3 family; external compatibility types may remain. Full-screen dialogs are deferred. |
