...
TEST(FltkKeySource, DrainsQueuedEventsInOrder) {
  ...
  EXPECT_TRUE(source.enqueue(
      {KeyPhase::kDown, KeyCode::kCharacter, kKeyModifierShift,
       PhysicalKey::kA, 'A'}));
  EXPECT_TRUE(source.enqueue(
      {KeyPhase::kUp, KeyCode::kTab, 0, PhysicalKey::kTab, 0}));
  ...
  EXPECT_EQ(PhysicalKey::kA, events[1].physical_key);
...
TEST(FltkKeySource, RejectsEventsAfterQueueCapacity) {
  FltkKeySource source;
  KeyEvent event{KeyPhase::kDown, KeyCode::kEnter, 0, PhysicalKey::kEnter,
                 0};
